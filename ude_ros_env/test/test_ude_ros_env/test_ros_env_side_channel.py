...
@patch("ude_ros_env.ros_env_side_channel.ServiceProxyWrapper")
@patch("ude_ros_env.ros_env_side_channel.rospy")
class ROSEnvironmentSideChannelTest(TestCase):
    def setUp(self) -> None:
        pass

    def test_initialize(self, rospy_mock, service_proxy_wrapper_mock):
        ros_side_channel = ROSEnvironmentSideChannel()
        rospy_mock.Service.assert_called_once_with(const.SideChannelServiceType.TO_ENV.value,
                                                   UDESideChannelSrv,
                                                   ros_side_channel.on_message_received)

        service_proxy_wrapper_mock.assert_called_once_with(const.SideChannelServiceType.TO_UDE.value,
                                                           UDESideChannelSrv)
